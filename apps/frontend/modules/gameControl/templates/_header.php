...
<p>
	...
	<?php if ($_isManager): ?>
		<?php if (($_game->status >= Game::GAME_STEADY) && ($_game->status <= Game::GAME_FINISHED)): ?>
			<span class="button-warn"><?php echo link_to('Пересчитать', 'gameControl/update?id='.$_game->id.'&returl='.$_retUrlRaw, array('method' => 'post')); ?></span>
		...
		<?php if ($_game->status == Game::GAME_PLANNED): ?>
			<span class="button-info"><?php echo link_to('Подготовить к запуску', 'gameControl/verify?id='.$_game->id.'&returl='.$_retUrlRaw, array('method' => 'post', 'confirm' => 'Подготовить игру '.$_game->name.' к запуску?')); ?></span>
		<?php elseif ($_game->status == Game::GAME_VERIFICATION): ?>
			<span class="button-info"><?php echo link_to('Повторить проверку', 'gameControl/verify?id='.$_game->id.'&returl='.$_retUrlRaw, array('method' => 'post', 'confirm' => 'Повторить предстартовую проверку игры '.$_game->name.'?')); ?></span>
		<?php elseif ($_game->status == Game::GAME_READY): ?>
			<span class="button-warn"><?php echo link_to('Запустить', 'gameControl/start?id='.$_game->id.'&returl='.$_retUrlRaw, array('method' => 'post', 'confirm' => 'Запустить игру '.$_game->name.'?')); ?></span>
			<span class="button-info"><?php echo link_to('Повторить проверку', 'gameControl/verify?id='.$_game->id.'&returl='.$_retUrlRaw, array('method' => 'post', 'confirm' => 'Повторить предстартовую проверку игры '.$_game->name.'?')); ?></span>
		<?php elseif (($_game->status == Game::GAME_STEADY) || ($_game->status == Game::GAME_ACTIVE)): ?>
			<span class="button-danger"><?php echo link_to('Завершить', 'gameControl/stop?id='.$_game->id.'&returl='.$_retUrlRaw, array('method' => 'post', 'confirm' => 'Остановить игру '.$_game->name.'?')); ?></span>
		<?php elseif ($_game->status == Game::GAME_FINISHED): ?>
			<span class="button-warn"><?php echo link_to('Сдать в архив', 'gameControl/close?id='.$_game->id.'&returl='.$_retUrlRaw, array('method' => 'post', 'confirm' => 'Игру больше нельзя будет редактировать! Вы уверены, что хотите сдать в архив игру '.$_game->name.'?')); ?></span>
		<?php endif; ?>
		<?php	if ($_game->status > Game::GAME_PLANNED): ?>
			<span class="button-danger"><?php echo link_to('Перезапустить', 'gameControl/reset?id='.$_game->id.'&returl='.$_retUrlRaw, array('method' => 'post', 'confirm' => 'Перезапустить игру '.$_game->name.'?'));?></span>
		<?php endif; ?>
	<?php endif; ?>
</p>
