...
<p>
	...
	<span class="info info-bg pad-box box"><?php echo link_to('Использовать автовыбор', 'gameControl/setNext?teamState='.$_teamState->id.'&taskId=0&returl='.$_retUrl, array ('method' => 'post')); ?></span>
	<?php else: ?>
		<?php if ($_teamState->task_id > 0): ?>
	<span class="warn warn-bg pad-box box"><?php echo link_to('Отменить выбор назначенного задания', 'gameControl/setNext?teamState='.$_teamState->id.'&taskId=0&returl='.$_retUrl, array ('method' => 'post')); ?></span>
		<?php endif ?>
	<?php endif ?>
</p>
		...
		<?php if ($_tasksInSequenceManual->count() > 0): ?>
			...
			<?php foreach ($_tasksInSequenceManual as $task): ?>
				<tr>
					...
					<td>
						<span class="info info-bg pad-box box"><?php echo link_to('Назначить', 'gameControl/setNext?teamState='.$_teamState->id.'&taskId='.$task->id.'&returl='.$_retUrl, array ('method' => 'post')) ?></span>
					</td>			
				</tr>
			<?php endforeach; ?>
		<?php endif ?>
		...
		<?php if ($_tasksNonSequence->count() > 0): ?>
			...
			<?php foreach ($_tasksNonSequence as $task): ?>
				<tr>
					...
					<td>
						<span class="warn warn-bg pad-box box"><?php echo link_to('Назначить', 'gameControl/setNext?teamState='.$_teamState->id.'&taskId='.$task->id.'&returl='.$_retUrl, array ('method' => 'post')) ?></span>
					</td>			
				</tr>
			<?php endforeach; ?>
		<?php endif ?>
