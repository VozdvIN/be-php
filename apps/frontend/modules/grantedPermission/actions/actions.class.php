...
class grantedPermissionActions extends MyActions
{
	...
	{
		parent::preExecute();
		$this->errorRedirectUnless($this->sessionWebUser->can(Permission::PERMISSION_MODER, 0), Utils::cannotMessage($this->sessionWebUser->login, 'просматривать список игр'));
	}
	...
	public function executeDelete(sfWebRequest $request)
	{
		...
		{
			$this->errorRedirect($wasDeny ? 'Снять запрет не удалось: '.$res : 'Отозвать право не удалось: '.$res);
		}

		$this->successRedirect($wasDeny ? 'Запрет успешно снят.' : 'Право успешно отозвано.');
	}
	...
	protected function processForm(sfWebRequest $request, sfForm $form)
	{
		...
		if ($form->isValid())
		{
			...
			{
				$this->successRedirect(($object->deny ? 'Запрет успешно установлен. ' : 'Право успешно назначено.'), 'webUser/show?id='.$object->web_user_id);
			}
		}
		else
		{
