...
class teamStateActions extends myActions
{
	...
	protected function processForm(sfWebRequest $request, sfForm $form)
	{
		...
		if ($form->isValid())
		{
			...
			$this->successRedirect(
				'Настройки команды '.$object->Team->name.' успешно сохранены.',
				'gameEdit/teams?id='.$object->game_id
			);
		}
		else
		{
	...
	public function executeAbandonTask(sfWebRequest $request)
	{
		...
		{
			$this->errorRedirect(
				'Не удалось отменить текущее задание команды '.$teamState->Team->name.' : '.$res,
				'gameControl/tasks?id='.$teamState->game_id
			);
		}
		$teamState->save();
		$this->successRedirect(
			'Текущее задание команды '.$teamState->Team->name.' успешно отменено.',
			'gameControl/tasks?id='.$teamState->game_id
		);
	}
	...
	public function executeForceFinish(sfWebRequest $request)
	{
		...
		{
			$this->errorRedirect(
				'Не удался принудительный финиш команды '.$teamState->Team->name.' : '.$res,
				'gameControl/tasks?id='.$teamState->game_id
			);
		}
		$teamState->save();
		$this->successRedirect(
			'Команда '.$teamState->Team->name.' принудительно финишировала.',
			'gameControl/tasks?id='.$teamState->game_id
		);
	}
}
